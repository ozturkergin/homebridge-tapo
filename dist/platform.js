...
import { deferAndCombine, delay, getAvailablePort, isObjectLike, loadPackageConfig, lookup, lookupCharacteristicNameByUUID, runCommand, satisfiesVersion, waitForServer, } from './utils.js';
...
export default class KasaPythonPlatform {
    ...
    async didFinishLaunching() {
        ...
        try {
            ...
            while (retries > 0) {
                try {
                    if (retries < 3) {
                        this.log.debug(`Retrying Kasa API startup... (${retries} retries left)`);
                        this.port = await getAvailablePort();
                        this.log.debug(`New port assigned: ${this.port}`);
                        if (this.deviceManager) {
                            this.deviceManager.refreshConfigSnapshot();
                        }
                    }
                    await this.startKasaApi();
                    await waitForServer(`http://127.0.0.1:${this.port}/health`, this.log, 10000, 1000, this.kasaProcess, this.apiToken);
                    break;
                }
                catch (error) {
                    this.log.warn(`Failed to start Kasa API on port ${this.port}: ${error}`);
                    this.stopKasaApi();
                    retries--;
                    if (retries === 0) {
                        throw error;
    ...
    async startKasaApi() {
        ...
        try {
            const [, , , kasaProcessInstance] = await runCommand(this.log, this.venvPythonExecutable, [scriptPath, this.port.toString(), this.hideHomeKitMatter.toString(), this.apiToken], { env: { ...process.env, KASA_API_TOKEN: this.apiToken } }, this.config.advancedOptions.advancedPythonLogging ? false : true, this.config.advancedOptions.advancedPythonLogging ? false : true, true);
    ...
    configureAccessory(accessory) {
        ...
        if (accessory.context.lastSeen) {
            ...
            if (timeSinceLastSeen > offlineInterval && accessory.context.offline === true) {
                this.log.debug(`Platform Accessory [${accessory.displayName}]${hostInfo} is offline and outside the offline interval, ` +
        ...
        if (!this.configuredAccessories.has(accessory.UUID)) {
            this.log.debug(`Platform Accessory [${accessory.displayName}]${hostInfo} with UUID [${accessory.UUID}] ` +
                'is not in configuredAccessories, adding it.');
            this.configuredAccessories.set(accessory.UUID, accessory);
        }
        else {
            this.log.debug(`Platform Accessory [${accessory.displayName}]${hostInfo} with UUID ` +
    ...
    async createHomeKitDevice(kasaDevice) {
