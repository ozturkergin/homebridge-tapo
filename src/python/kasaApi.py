...
async def broadcast_device_event(device_data: dict[str, Any]):
    for queue in list(sse_subscribers):
        await queue.put(device_data)

@app.before_request
async def authenticate():
    expected_token = os.getenv("KASA_API_TOKEN")
...
async def discover_devices(
    ...
    async def on_discovered(device: Device):
        ...
        try:
            ...
            if result is not None:
                await broadcast_device_event(result)
    ...
    await broadcast_device_event({"status": "discovery_complete"})
...
ALLOWED_ACTIONS: dict[str, set[str]] = {
    ...
    "fan_speed_level": {"set_fan_speed_level"},
    "hsv": {"set_hsv"},
}
...
async def perform_device_action(
    ...
    try:
        if feature not in ALLOWED_ACTIONS or action not in ALLOWED_ACTIONS[feature]:
            raise ValueError(f"Invalid feature '{feature}' or action '{action}'")
        ...
        if feature == "state":
            ...
            await handle_brightness(target, value)
        elif feature == "color_temp" and light and light.has_feature("color_temp"):
            await handle_color_temp(target, value)
        elif feature == "fan_speed_level" and fan and fan.has_feature("fan_speed_level"):
            await handle_fan_speed_level(target, value)
        elif feature == 'hsv' and light and light.has_feature("hsv"):
            await handle_hsv(target, value)
        else:
            raise ValueError("Invalid feature or device does not support feature")
        return {"status": "success"}
    except Exception as e:
        log(f"Error performing action: {e}", level="ERROR", alias=device.alias)
        return {"status": "error", "message": str(e)}

async def handle_brightness(target: Device, value: int):
    log(f"Handling brightness: value={value}", alias=target.alias)
    light = target.modules.get(Module.Light)
    if value == 0:
        await target.turn_off()
        return
...
async def handle_color_temp(target: Device, value: int):
    log(f"Handling color temperature: value={value}", alias=target.alias)
    light = target.modules.get(Module.Light)
    color_temp = target.modules.get(Module.ColorTemperature)
    mired = max(140, min(int(value), 500))
...
async def handle_fan_speed_level(target: Device, value: int):
    log(f"Handling fan speed level: value={value}", alias=target.alias)
    fan = target.modules.get(Module.Fan)
    level = _percent_to_level(int(value))
    if level == 0:
        await target.turn_off()
        return
    await fan.set_fan_speed_level(level)
    if target.is_off:
        await target.turn_on()

async def handle_hsv(target: Device, value: dict):
    log(f"Handling HSV: value={value}", alias=target.alias)
    light = target.modules.get(Module.Light)
    current_hsv = light.hsv
    h = value.get("hue", current_hsv[0])
    s = value.get("saturation", current_hsv[1])
    v = current_hsv[2]
    new_h = max(0, min(int(h), 360))
    new_s = max(0, min(int(s), 100))
    new_v = max(0, min(int(v), 100))
    await light.set_hsv(new_h, new_s, new_v)
